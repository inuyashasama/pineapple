...
public class PointRecordController {
    ...
    @PostMapping("/earn")
    public Result<String> earn(@RequestBody PointRecord dto) {
        pointService.addPoints(dto.getUserId(), dto.getPoints(), dto.getReason(), dto.getOrderId());
        return Result.ok("积分已发放");
    }

    @PostMapping("/use")
    public Result<String> use(@RequestBody PointRecord dto) {
    ...
    @GetMapping("/{userId}")
    public Result<Integer> get(@PathVariable Long userId) {
        Integer points = pointService.getUserPoints(userId);
        return Result.ok(points);
    }

    @GetMapping("/getSignStatus/{userId}")
    public Result<Boolean> getSignInStatus(@PathVariable Long userId) {
        boolean signed = pointService.isSigned(userId);
        return Result.ok(signed);
    }

    @GetMapping("/getUserPointsHistory/{userId}")
    public Result<List<PointRecord>> getUserPointsHistory(@PathVariable Long userId) {
